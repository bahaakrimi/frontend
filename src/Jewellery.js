...
function Jewellery() {
  ...
  useEffect(() => {
      const fetchBestProduct = async () => {
        try {
          const response = await fetch('http://localhost:5000/api/recommendations/jewellery');
  ...
  useEffect(() => {
    const fetchJeux = async () => {
      try {
        ...
        const jeuxProduits = response.data.filter(produit => produit.category === 'jewellery');
  ...
  const jewelleryProduits = produits.filter(produit => produit.category === 'jewellery');
  // Style JSX intégré
  const styles = {
    banner: {
      textAlign: 'center',

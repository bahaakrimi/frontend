...
function Jewellery() {
  ...
  useEffect(() => {
      const fetchBestProduct = async () => {
        try {
          const response = await fetch('http://localhost:5000/api/recommendations/jeux');
  ...
  const jewelleryProduits = produits.filter(produit => produit.category === 'jeux');
  // Style JSX intégré
  const styles = {
    banner: {
      textAlign: 'center',
        ...
        <div id="jewellery_main_slider" className="carousel slide" data-ride="carousel">
          <div className="carousel-inner">
            <div className="carousel-item active">
              <div className="container">
                <h1 className="fashion_taital">🎮 Jeux videos & Consoles</h1>
